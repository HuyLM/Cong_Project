...
public static class UnityGoogleDriveHelper {
    /// <summary>
    ...
    public const string DocummentMimeExportType = "text/plain";
            ...
            for (int i = 0; i < parentNames.Length; i++) {
                using (var listRequest = new GoogleDriveFiles.ListRequest()) {
                    ...
                    if (listRequest.ResponseData.Files == null || listRequest.ResponseData.Files.Count == 0) {
                        var folder = new UnityGoogleDrive.Data.File { Name = parentNames[i], MimeType = FolderMimeType, Parents = new List<string> { parentId } };
    ...
    public static async System.Threading.Tasks.Task<HashSet<string>> ValidatePath(string path, bool appData, string parentId = null) {
        ...
        if (string.IsNullOrWhiteSpace(path))
            return new HashSet<string> { parentId };
        path = Path.GetDirectoryName(path).Replace('\\', '/');
    ...
    public static async System.Threading.Tasks.Task<UnityGoogleDrive.Data.File> GetFileWithContent(string id, string mimeType) {
        GoogleDriveFiles.ExportRequest export = GoogleDriveFiles.Export(id, mimeType);
        await export.Send();
        return export.ResponseData;
    }
}
